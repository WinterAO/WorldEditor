...
Private Const CHANNEL_AMBIENT   As Long = 2
...
Private AmbientEnabled_ As Boolean
Private AmbientID_      As Long
Private AmbientVolume_  As Long
...
Public Sub StopAmbient()
    Call Cancel(AmbientID_)
End Sub
...
Public Function PlayEffect(ByVal Filename As String, Optional ByVal Emitter As Audio_Emitter = Nothing, Optional ByVal Repeat As Boolean = False) As Long
    ...
    PlayEffect = Play(CHANNEL_EFFECT, Filename, Emitter, Repeat, True)
    
End Function

Public Function PlayAmbient(ByVal Filename As String) As Long

    If Not AmbientEnabled_ Then Exit Function

    Call Cancel(AmbientID_)

    AmbientID_ = Play(CHANNEL_AMBIENT, Filename, Nothing, False, False)
...
Public Property Get AmbientEnabled() As Boolean

    AmbientEnabled = AmbientEnabled_
    
End Property

Public Property Let AmbientEnabled(ByVal Activate As Boolean)

    If AmbientEnabled_ = Activate Then Exit Property

    AmbientEnabled_ = Activate

    If Activate Then
        Call Aurora_Audio.SetSubmixVolume(CHANNEL_AMBIENT, AmbientVolume_ * 0.01)
    Else
        Call Aurora_Audio.SetSubmixVolume(CHANNEL_AMBIENT, 0)
    End If
    
End Property

Public Property Let AmbientVolume(ByVal Volume As Long)

    If Volume < 0 Or Volume > 100 Then Exit Property

    Call Aurora_Audio.SetSubmixVolume(CHANNEL_AMBIENT, Volume * 0.01)

    AmbientVolume_ = Volume
    
End Property

Public Property Get AmbientVolume() As Long

    AmbientVolume = Aurora_Audio.GetSubmixVolume(CHANNEL_AMBIENT) * 100
    
End Property

Private Function Play(ByVal Channel As Long, ByVal Filename As String, ByVal Emitter As Audio_Emitter, ByVal Repeat As Boolean, ByVal isEffect As Boolean) As Long

    Dim TypeSound As String
    ...
    Set Effect = Aurora_Content.Load("Resources://" & TypeSound & Filename, eResourceTypeSound)
